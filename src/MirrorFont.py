...
class MirrorFont(Texture):
  ...
  def __init__(self, font, color=(255,255,255,255), codepoints=None,
    ...
    *italic_adjustment*:
    ...
    """
    super(MirrorFont, self).__init__(font)
    self.font = font
    try:
    ...
    while image_size < MAX_SIZE and not all_fits:
      self.im = Image.new("RGBA", (image_size, image_size), 128)
      self.alpha = True
      ...
      for i in itertools.chain([0], codepoints):
        ...
        # Draw the text on a temporary image
        chrim = Image.new("RGBA", (chwidth+pipew, int(curY)), background_color)
        chrimd = ImageDraw.Draw(chrim)
        chrimd.text((0, 0), chstr, font=imgfont, fill=color)
        chrimd.rectangle([(0,0),(10,10)], fill=color)
    ...
    self.image = self.im.transpose(Image.FLIP_LEFT_RIGHT).convert(RGBs).tostring('raw', RGBs)
    self._tex = ctypes.c_int()

    # Translate the glyph table coordinates for the transposed
    # image
    ngt = {}
    for ch, g in self.glyph_table.items():
      c = g[2]
      g[2] = [[image_size-c[1][0]-1, c[1][1]], 
             [image_size-c[0][0]-1, c[0][1]],
             [image_size-c[3][0]-1, c[3][1]],
             [image_size-c[2][0]-1, c[2][1]]]
